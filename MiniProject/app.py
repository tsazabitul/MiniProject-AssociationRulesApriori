...
def load_and_clean_data(uploaded_file) -> pd.DataFrame:
    ...
    if uploaded_file.name.endswith('.xlsx'):
        ...
        df = pd.read_csv(
            uploaded_file,
            encoding='ISO-8859-1',  # Dataset Online Retail sering menggunakan encoding ini
            dtype={'CustomerID': str, 'InvoiceNo': str}  # Paksa tipe str agar tidak kehilangan leading zero
        )

    # ── Cleaning Steps (identik dengan notebook) ──
...
with st.sidebar:
    ...
    uploaded_file = st.file_uploader(
        ...
        type=['csv', 'xlsx'],
        help="Format: Online Retail dataset dengan kolom InvoiceNo, Description, Country, Quantity, UnitPrice, dst."
    )

    if uploaded_file is None:
        st.info("⬆️ Silakan upload file CSV terlebih dahulu.")
        st.stop()   # Hentikan eksekusi jika belum ada file
    ...
    # ──────────────────────────────────────────────────────────
    st.markdown("### ⚙️ Parameter Apriori")
    ...
    st.markdown("---")
    st.caption("💡 Tips: Mulai dengan support=0.02 dan confidence=0.3, lalu sesuaikan.")
...
<div class="main-header">
    ...
    <p>Temukan pola pembelian tersembunyi dengan Algoritma Apriori</p>
</div>
""", unsafe_allow_html=True)
...
if st.session_state.get('analysis_run', False):
    ...
    # ──────────────────────────────────────────────────────────
    with tab1:
        st.markdown("#### 🗂️ Semua Aturan Asosiasi")
            ...
            if not product_rules.empty:
                ...
                for _, row in product_rules.iterrows():
                    ...
                    <div class="rule-highlight">
                        <strong>➡️ {row['consequents_str']}</strong><br>
                        <small>
    ...
    with tab4:
        ...
        with col_fi1:
            st.dataframe(
                fi_display[['itemsets_str', 'support', 'itemset_length']].rename(columns={
                    'itemsets_str': 'Itemset',
                    'support': 'Support',
                    'itemset_length': 'Jumlah Item'
                }),
                use_container_width=True,
                height=400,
                hide_index=True
